...
class Magentomasters_Supplier_Model_Output{
		...
		public function getPdfReport($dropshipment_ids){
			...
			foreach ($dropshipsuppliers as $dropshipsupplier){
				...
				$finalpdf = $this->_runParser($processor->filter($templates['pdfreport'])); 	
		...
		public function getEmail($order_id,$supplier_id,$items,$trigger){
			...
			if($supplier['email_attachement'] == 2 && $supplier['xml_enabled'] == 1 && $supplier['xml_csv'] == 1){
				...
				if($file){
					...
					$at2->filename = "order_" . $orderIncrementId . ".csv";
				} else { 
			...
			if($supplier['email_attachement'] == 1 && $supplier['xml_enabled'] == 1 && $supplier['xml_csv'] == 0){
				...
				if($file){
					...
					$at3->filename = "order_" . $orderIncrementId . ".xml";
				} else {
			...
			if($supplier['email_attachement'] == 1 && !$supplier['xml_type'] || $supplier['email_attachement'] == 2 && !$supplier['xml_type']){
				try{
					unlink($file);
					Mage::getModel('supplier/observer')->logging("Attachement Deleted");
				} catch(exception $e){
					Mage::getModel('supplier/observer')->logging($e->getMessage());
				}
			}
		...
		private function processSubject($subject,$order){
			...
			$processor = Mage::helper('cms')->getBlockTemplateProcessor();		
			$processor->setVariables($variables);				
			return $this->_runParser($processor->filter($subject));		
		}
		...
		private function processTemplate($order,$supplier,$items,$type){
			...
			if($type=='pdf'){
				$processor->setVariables($variables);				
				$finaltemplate = $this->_runParser($processor->filter($templates['pdf']));		
			} elseif($type=='email') {
				$processor->setVariables($variables);
				$finaltemplate = $this->_runParser($processor->filter($templates['email']));				
			} elseif($type=='xml'){
				$processor->setVariables($variables);							
				$finaltemplate = $this->_runParser($processor->filter($templates['xml']));		
			} elseif($type=='xmlnew'){
		...
		private function processitemTemplate($template,$items,$type,$templatevariables,$supplier){	
			...
			foreach($items as $item){
				...
				$variables = array(
					...
					'qty' => round($item->getQtyOrdered() - $item->getQtyRefunded()),
					'options' => $customOptionsInfo,
					'xml_options' => $xmlcustomOptionsInfo,
					'finalprice' => $item->getPrice() * $item->getQtyOrdered(), 
					'cost' => Mage::helper('core')->currency(($item->getBaseCost() * $item->getQtyOrdered()), true, false)	 
				);	 
				...
				if($type=='csv'){
					$attributes = explode('||', $this->_runParser($processor->filter($template)));
					$finaltemplate[] = $attributes;
				} else {
					$finaltemplate .= $this->_runParser($processor->filter($template));	
				}
			}
		...
		private function getTotalCosts($items){
			$cost = 0;	
			foreach($items as $item){
				$cost += $item->getBaseCost() * $item->getQtyOrdered();
			}
			return round($cost,2);
		}
		
		private function getInvoiceNumber($order){
			$invoiceIncrementId = "Noinvoice";	
		...
		private function _runParser($string){
			return Mage::helper('supplier/parser')->parse($string);
		}
