<?php class Magentomasters_Supplier_Model_Schedule extends Mage_Core_Model_Abstract {
	...
	public function importStock($source,$supplierid){
		...
		$skufield = strtolower($supplier['schedule_import_stock_sku']); 
		$qtyfield = strtolower($supplier['schedule_import_stock_qty']);
		...
		if($type==1 && $url && $divider){
			...
			$file = fopen($url, "r");
			if($file){
				while (($result = fgetcsv($file,0,$divider)) !== false)
				{
	    			$csv[] = $result;
				}
				fclose($file);
			...
			foreach ($headers as $key => $value) {
				$mapping[strtolower($value)] = $key;
				$checkmapping[strtolower($value)] = $key + 1;
			}

  			unset($csv['0']); // Remove headers
			if(isset($checkmapping[$qtyfield]) && isset($checkmapping[$skufield])){
				foreach ($csv as $item) {
