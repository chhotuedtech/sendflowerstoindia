...
class Magentomasters_Supplier_Model_Supplier extends Mage_Core_Model_Abstract {
	...
	public function getProductRelations($supplier){
		$option = $this->getSupplierOptionsId($supplier);
		$connect = Mage::getSingleton('core/resource')->getConnection('core_read');
		$table = Mage::getSingleton('core/resource')->getTableName('catalog_product_entity_int');
		$query = "SELECT entity_id FROM " . $table . " WHERE attribute_id='".$option['attribute_id']."' AND value='" . $option['option_id'] . "'";	
		$result = $connect->query($query);
		$result = $result->fetchAll();
		return $result;
	}
	
	public function saveSupplierProducts($products,$supplier){
		$option = $this->getSupplierOptionsId($supplier);
		$connect = Mage::getSingleton('core/resource')->getConnection('core_read');
		$table = Mage::getSingleton('core/resource')->getTableName('catalog_product_entity_int');	
		foreach($products as $productId){
			$query = "UPDATE ". $table ." SET value=".$option['option_id']." WHERE attribute_id='".$option['attribute_id']."' AND entity_id='" . $productId . "'";	
			$connect->query($query);
			//$product = Mage::getModel('catalog/product')->load($productId)->setSupplier($option['option_id'])->save(); 
		}
	}
}