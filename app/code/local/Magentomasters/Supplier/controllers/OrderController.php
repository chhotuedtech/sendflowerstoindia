<?php
class Magentomasters_Supplier_OrderController extends Magentomasters_Supplier_Controller_Supplier {
    ...
    public function addcommentAction(){
        ...
        if( $supplierId && $supplierId != "logout") {	
	        ...
	        if ($post) {
	            $comment = trim(strip_tags($post['history']['comment']));
	            $order->addStatusToHistory($order->getStatus(), $comment, $post['history']['is_customer_notified']);
	            $order->save();
	            $order->sendOrderUpdateEmail($post['history']['is_customer_notified'], $comment);
	        }
			Mage::getSingleton('core/session')->addSuccess($this->__('Comment added'));
	        $this->_redirect('supplier/order/history',array('order_id'=>$orderId));
    	} else {
            $redirectPath = Mage::getUrl() . "supplier/";
            $this->_redirectUrl( $redirectPath );
        }
